...
int authenticateAdmin(){        //function to authenticate the admin.
    char username[15], password[6];
    
    printf("\nEnter username: ");       //Inputing the username from the admin.
    scanf("%s",username);
    if((strcmp(username,"Admin"))!=0)  //checking the inputted username
        return 0;
    else
    {
        printf("Enter Password: "); //inputing the password from the admin
        int i=0;
        for(i=0;i<5;i++)
        {
            password[i]=getch();
...
void banID(){       //function to ban  a user ID
    printf("\nCreating Banned.txt...\n"); 
    FILE *fp=fopen("Banned.txt", "w"); //opening a new file nammed banned.txt
    if(fp==NULL){
        printf("Error: Banned.txt not created.\n");
        fclose(fp);
        return;
    }
    printf("Just Enter last roll no to ban\nPress 0 to exit... ");  //Inputing the roll no. of the student.
    int input;
    while(1){
        printf("\nEnter Number: ");
        scanf("%d",&input);
    ...
    fclose(fp); //clsing the file.
    printf("Created Successfully\n");
}

void createCandidateFiles(){        //function to create files of the candidates
...
//STUDENT PANEL
int isValid(char userID[15])        //function to check whether user ID is valid or not.
{
    if(strlen(userID)!=14)
        return 0;
...
int isVoted(char userID[15])        //function to check whether the user has voted or not.
{
    int location=extractRollNo(userID);
...
int isBanned(char userID[15]){      //function to check whether the user ID is banned or not.
    int location=extractRollNo(userID);
...
void saveVote(char userID[15],char voteInput)       //function to save vote of the user
{
    char filename[20];
...
void studentPanel()  // function for main student panel
{
    char userID[15];
    char voteInput;
    while(1)
	{
		printf("\n\n  To exit press 0");
        printf("\n  Enter user ID:");       //inputting user ID from the user.
        scanf("%s",userID);
        if(strcmp(userID, "0")==0)
				return;
        if(isValid(userID)!=1)  //calling function isValid to check whether the user ID is correct or not.
        {
            printf("\n  Invalid User ID(Press Enter)");
            getch();
            continue;
        }
        if(isBanned(userID)!=0)     //calling function isBanned to check whether the user is banned or not.
        {
            printf("\nThis User ID is currently banned...\nContact Admin for the reason...(Press Enter to continue)");
            getch();
            continue;
        }
        if(isVoted(userID)!=0)      //calling function isVoted to check whether the user has voted or not.
        {
            printf("\n  Your PRN entered is already voted\n  Contact Admin for furthur query");
            getch();
            continue;
        }
        printf("\n\n  Candidates for election:");       //printing the names of the canidates.
		for (int i = 0; i < numberOfCandidates; i++)
        {
